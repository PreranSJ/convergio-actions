...
class CommerceWebhookController extends Controller
{
        ...
        switch ($eventType) {
            ...
            case 'checkout.session.completed':
                ...
                if (isset($data['mode']) && $data['mode'] === 'subscription') {
                    return $this->handleSubscriptionCheckoutCompleted($event, $transaction);
                }
                return $this->handleCheckoutSessionCompleted($data, $transaction);
            ...
            case 'invoice.finalized':
                return $this->handleSubscriptionEvent($event, $transaction);
    ...
    /**
     * Handle subscription checkout completed event.
     */
    private function handleSubscriptionCheckoutCompleted(array $event, CommerceTransaction $transaction): array
    {
        try {
            $tenantId = $this->extractTenantIdFromEvent($event);
            if (!$tenantId) {
                Log::channel('commerce')->error('No tenant ID found in subscription checkout event');
                return ['status' => 'no_tenant_id'];
            }

            $this->subscriptionService->handleStripeWebhookEvent($tenantId, $event);

            return ['status' => 'subscription_checkout_processed'];
        } catch (\Exception $e) {
            Log::channel('commerce')->error('Failed to handle subscription checkout completed', [
                'error' => $e->getMessage(),
                'event_id' => $event['id'] ?? null,
            ]);
     ...
     */
    private function handleSubscriptionEvent(array $event, CommerceTransaction $transaction): array
    {
        try {
            $tenantId = $this->extractTenantIdFromEvent($event);
            if (!$tenantId) {
                Log::channel('commerce')->error('No tenant ID found in subscription event');
                return ['status' => 'no_tenant_id'];
            }

            $this->subscriptionService->handleStripeWebhookEvent($tenantId, $event);

            return ['status' => 'subscription_event_processed'];
        } catch (\Exception $e) {
            Log::channel('commerce')->error('Failed to handle subscription event', [
                'error' => $e->getMessage(),
