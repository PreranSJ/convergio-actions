...
class CommercePaymentLinkFactory extends Factory
{
    ...
    {
        return [
            'quote_id' => null, // Will be set by withQuote() state if needed
            'order_id' => null,
    ...
    /**
     * Indicate that the payment link is cancelled.
     */
    ...
    public function test(): static
    {
        return $this->state(fn (array $attributes) => [
            ...
            'public_url' => 'https://example.com/test-payment/' . Str::random(10),
        ]);
    }
    ...
    public function real(): static
    {
        return $this->state(fn (array $attributes) => [
            'stripe_session_id' => 'cs_test_' . Str::random(24),
            'public_url' => 'https://checkout.stripe.com/c/pay/' . Str::random(20),
        ]);
    }

    /**
     * Indicate that the payment link has a quote.
     */
    ...
    /**
     * Indicate that the payment link has an order.
     */
